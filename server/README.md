...
Provider references: [fal queue API](https://fal.ai/docs/documentation/model-apis/inference/queue), [Sunburst Edit schema](https://fal.ai/models/openai/gpt-image-2.5/sunburst/edit/api), [Google backend ID-token verification](https://developers.google.com/identity/gsi/web/guides/verify-google-id-token). TripoSplat compatibility was checked against Avalon's live `/gradio_api/info` and `run_gradio.py`.

Image generation uses `openai/gpt-image-2.5/sunburst/edit`: the saved snapshot is the sole reference image, with `quality: "medium"` and 1024×1024 PNG output. Depth and wireframe remain saved with each job but are not sent to the image model. Update any existing `WS_FAL_MODEL` override to this endpoint; the request schema targets Sunburst Edit.
