...
Provider references: [fal queue API](https://fal.ai/docs/documentation/model-apis/inference/queue), [FLUX depth conditioning schema](https://fal.ai/models/fal-ai/flux-control-lora-depth/api), [Google backend ID-token verification](https://developers.google.com/identity/gsi/web/guides/verify-google-id-token). TripoSplat compatibility was checked against Avalon's live `/gradio_api/info` and `run_gradio.py`.

Image generation uses `fal-ai/flux-control-lora-depth`: the saved snapshot is the image-to-image input and the saved depth map is the control image, with depth preprocessing disabled and control strength 1. Output is 1024×1024 PNG. Update any existing `WS_FAL_MODEL` override when switching from Klein; the request schema now targets the depth endpoint.
